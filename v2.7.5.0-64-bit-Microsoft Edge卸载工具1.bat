...
cd /d “％〜dp0”
中立证券
reg 添加“HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Microsoft Edge Update”/v NoRemove /t REG_DWORD /d 0 /f
reg 添加“HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Microsoft EdgeWebView”/v NoRemove /t REG_DWORD /d 0 /f
reg 添加“HKEY_LOCAL_MACHINE\SOFTWARE\WOW6432Node\Microsoft\Windows\CurrentVersion\Uninstall\Microsoft Edge”/v NoRemove /t REG_DWORD /d 0 /f
中立证券
%b2eincfilepath%/install_wim_tweak.exe /o /l
%b2eincfilepath%/install_wim_tweak.exe /o /c Microsoft-Windows-Internet-浏览器包 /r
...
cls & set "现在请打开Edge测试你的Edge是否能够打开：=" & echo 能打开（1）& echo 无法/提示文件损坏（2） & echo 试过更改字母，仍然无效（3）
set /P a=现在请打开Edge测试你的Edge是否能够打开：
if "%a%"=="1" cls & echo 请重新卸载 & Pause & cls & goto start & exit
if "%a%"=="2" cls & echo 即将开始清除樱桃
if "%a%"=="3" cls & set "您可能是MSI版Edge浏览器，需保证网络稳定，并且有≥800MB的系统盘剩余空间保证所需程序能正常调用（此文件在卸载完成Edge后卸载完成会自动删除）=" & echo 开始卸载（Y） & echo 取消卸载（N） & set /P msi=您可能是MSI版Edge器浏览，需保证网络稳定，且有≥800MB的系统盘剩余空间以保证所需程序能正常调用（此文件在卸载Edge后卸载完成会自动删除）：
if "%msi%"=="Y" cls & echo 正在下载降级包.... & certutil -URLCache -f "https://dlink.host/sharepoint/aHR0cHM6Ly9taW5neHVhbmtlbGFuLW15LnNoYXJlcG9pbnQuY29tLzp1Oi9nL3BlcnNvbmFsL21pbmd4dWFua2VsYW5 fbWluZ3h1YW5rZWxhbl9vbm1pY3Jvc29mdF9jb20vRVh2ckVGUUxHdVZGdktYV1lPLU9fYmdCbmxYQzhRajhqbHdhQWpYQXQwZTVlUT9lPW5Mb3R5aQ.msi" "%SystemDrive%\MSi版Edge卸载所需程序.msi" & 复制 %SystemDrive%\MSi版Edge卸载所需程序.msi %SystemDrive%\MSi版Edge卸载所需程序.txt & msiexec /i %SystemDrive% \MSi版Edge卸载所需程序.msi /l* %SystemDrive%\MSi版Edge卸载所需程序.txt alloweddowngrade= 1 & echo 降级包安装完成，在控制面板卸载即可（点击删除残留将自动删除，请勿手动删除，否则无法卸载！如果卸载时出错，则需要先修复，再卸载），以继续删除残留 & rundll32.exe shell32.dll,Control_RunDLL appwiz.cpl & @pause
